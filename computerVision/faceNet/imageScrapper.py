...
# Target sites dict. Class/Dir is key, list of URLs is value:
targetsDict = {"Class 1": ["https://www.somepage.com/class1/gallery"],
               "Class 2": ["https://www.somepage.com/class2/gallery1", "https://www.somepage.com/class2/gallery2"]}

# Some stats are stored here:
statCounters = ["Downloaded", "Duplicates", "Written"]
statsDict = {}
...
for currentName in targetsDict:

    print("[>] Now Processing: " + currentName)

    # Create sub-dict for current class name:
    statsDict[currentName] = {}

    # Prepare stats dict entry:
    # statsDict = {Name: {Counter1:0.0, Counter2:0.0}}
    for c in range(len(statCounters)):
        # Get counter name:
        statsName = statCounters[c]
        # Create entry in dict, initialize the counter:
        statsDict[currentName][statsName] = 0.0
    ...
    if downloadImages:
        ...
        # Get the Target Sites list:
        targetSites = targetsDict[currentName]
        totalTargetSites = len(targetSites)
        print("Target: " + currentName + " Total Target Sites: " + str(totalTargetSites))

        # Images download counter:
        totalDownloaded = 0

        for s, currentSite in enumerate(targetSites):
            print("[!] Attempting to download images from: " + currentSite + " [" + str(s + 1) + "/" + str(
                totalTargetSites) + "]")
            ...
            while navigateGallery:
                ...
                for i, link in enumerate(links):
                    ...
                    # The target link has the "a" attribute and the "full"
                    # keyword:
                    images = soup.find_all("a", href=re.compile("full"))
                    ...
                    for image in images:
                        ...
                        # Get image link via the "original" attribute:
        ...
        # Print the downloaded images so far:
        print("Total downloaded images:", totalDownloaded)

        # Into stats dict:
        statsDict[currentName]["Downloaded"] = totalDownloaded
    ...
    if removeDuplicates:
        ...
        # Into stats dict:
        statsDict[currentName]["Duplicates"] = deleteCounter
        ...
        # Into stats dict:
        statsDict[currentName]["Written"] = newImagesCounter

print("\nStats Summary:")
for currentName in statsDict:
    # Get stats for this dir/class:
    currentStats = statsDict[currentName]
    currentDownloaded = currentStats["Downloaded"]
    currentDuplicates = currentStats["Duplicates"]
    currentWritten = currentStats["Written"]
    
    # Pretty print the detectors stats:
    print('{0:<22}: {1:<18} {2:<15} {3:<10}'.format("[" + currentName + "]",
                                                    "Downloaded: " + str(currentDownloaded),
                                                    "Duplicates: " + str(currentDuplicates),
                                                    "Written: " + str(currentWritten)))
